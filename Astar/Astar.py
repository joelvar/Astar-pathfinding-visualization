...
resetimg = pygame.image.load('Reset.png').convert_alpha()
...
font = pygame.font.SysFont('calibri', 26,bold=True,italic=True)
font2 = pygame.font.SysFont('calibri', 16)
text1 = font.render('A* Path Finding', True, BLACK)
text2 = font.render('Visualisation', True, BLACK)
...
button1 = Button(40, 125, startimg, 0.35)
button2 = Button(40, 300, resetimg, 0.35)
...
def drawgrid():
    ...
    for i in range(COL):
        pygame.draw.line(win, DGREY, (i * GAP + BORDER, 0),(i * GAP + BORDER, LENGTH))
...
def construct(came_from, current, draw):
    ...
    while not Q.empty():
        ...
        for neighbor in current.neighbor:
            ...
            if temp_g < g_score[neighbor]:
                ...
                f_score[neighbor] = g_score[neighbor] + dist(neighbor.get_pos(), end.get_pos())
...
def main():
    ...
    while run:
        ...
        if button1.func == True and count > 1:
            for row in grid:
                for node in row:
            ...
            alg(lambda: draw(grid), grid, start, end)
        
        if button2.func == True:
            count = 0
            button2.func = False
            start = None
            end = None
...
class Node:
    ...
    def update_neighbor(self, grid):
        self.neighbor = []
        if self.row < ROW - 1 and not grid[self.row + 1][self.col].is_barrier():
            self.neighbor.append(grid[self.row + 1][self.col])

        if self.row > 0 and not grid[self.row - 1][self.col].is_barrier():
            self.neighbor.append(grid[self.row - 1][self.col])

        if self.col < COL - 1 and not grid[self.row][self.col + 1].is_barrier():
